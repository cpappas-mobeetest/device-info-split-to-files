package com.mobeetest.worker.services.wiFi_drive
...
class WiFiDriveService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if (action in listOf("RUN", "PAUSE", "STOP")) {
            ...
            val broadcastIntent = Intent("com.mobeetest.worker.SERVICES_STATES").apply {
                ...
                putExtra("service_name", "WiFi Drive")
                setPackage(packageName)
            }
            sendBroadcast(broadcastIntent)
            Log.d("WiFiDriveService", "Broadcast sent with action $action")
        }
